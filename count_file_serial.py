...
def main():
    df = pd.read_csv('CORIOLIS/download/Global_floats.txt', header=None, sep=',')
    ...
    dir_path = 'CORIOLIS/download/tmp'
    ...
    df_downloaded_not_in_float_index.to_csv('files_downloaded_but_not_in_float_index.csv', index=False)

    df_in_float_index_not_downloaded = pd.DataFrame(in_float_index_not_downloaded, columns=['Filename'])
    df_in_float_index_not_downloaded.to_csv('files_in_float_index_BUT_not_downloaded.csv', index=False)
