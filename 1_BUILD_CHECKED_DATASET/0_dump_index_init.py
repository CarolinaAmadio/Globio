...
def argument():
    ...
    * lon,lat,time, parameters *
    ...
    parser.add_argument(   '--output_csv','-ocsv',
                                type = str,
                                required = True,
                                help = ''' generally named BadPosition_latlon.csv and stored in the onlinerepo
                                ''')
...
def file_header_content(filename,VARLIST, output_csv, avail_params=None):
    '''
    ...
    if BadPosition:
        data = {
            ...
            "latitude": [lat],
            "longitude": [lon]}
        line_append = pd.DataFrame(data)
        
        if os.path.exists(output_csv):
            df         = pd.read_csv(output_csv)
            updated_df = pd.concat([df, line_append], ignore_index=True)
        else:
            df         = pd.DataFrame(columns=["Namefile","latitude","longitude"])
            updated_df = line_append
       
        updated_df.drop_duplicates(subset="Namefile", keep="last", inplace=True)
        ...
        if filename[-4:]!='D.nc':
            ...
            if filename in FILELIST:
                ...
                if timedist.days > 15:
                    ...
                    line=file_header_content(filename,VARLIST, args.output_csv , avail_params=None)
                    if line is not None:
